        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-coindev-address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CCoindevAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CCoindevAddress(address).ToString());
}

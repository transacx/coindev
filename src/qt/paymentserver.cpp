...
const int COINDEV_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString COINDEV_IPC_PREFIX("coindev:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(COINDEV_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(COINDEV_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(COINDEV_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click coindev: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start coindev: click-to-pay handler");
    else
...
{
    // clicking on coindev: URLs creates FileOpen events on the Mac:

...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coindev: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coindev Signed Message:\n";
...
{
    // Coindev: IsDust() detection disabled, allows any valid dust to be relayed.
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 CDC if made of 50 CDC inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Coindev: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Coindev: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Coindev: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coindev: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coindev: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Coindev: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Coindev: increase each by adding 2 to coindev's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoindevMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoindevMiner : ProcessBlock, block not accepted");
    }
